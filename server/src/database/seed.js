...
const ADMIN_EMAIL = (
  process.env.SEED_ADMIN_EMAIL ?? 'admin@kick.com'
).toLowerCase();
...
const CATEGORIES = [
  {
    ...
    description:
      'Road and trail shoes built for distance, speed, cushioning and daily marathon training.',
  },
  {
    name: 'Sneakers',
    color: 'teal',
    ...
    description:
      'Everyday low-tops, high-tops, retro classics and streetwear lifestyle silhouettes.',
  },
  {
    name: 'Casual shoes',
    color: 'indigo',
    ...
    description:
      'Comfortable loafers, slip-ons, and daily walking shoes for street ease.',
  },
  {
    name: 'Formal',
    color: 'slate',
    ...
    description:
      'Oxfords, derbies, monk straps and leather dress shoes for the office and black-tie events.',
  },
  {
    name: 'Boots',
    color: 'amber',
    ...
    description:
      'Hiking, chelsea, waterproof and insulated weatherproof boots for rugged terrain and winter rain.',
  },
  {
    name: 'Basketball',
    color: 'orange',
    ...
    description:
      'High-traction court shoes built for explosive vertical lift, ankle support and court grip.',
  },
  {
    name: 'Outdoor',
    color: 'teal',
    ...
    description:
      'Rugged all-weather trail footwear designed for trail running, hiking and wet rainy seasons.',
  },
  {
    name: 'Training',
    color: 'rose',
    ...
    description:
      'Crossfit, gym workout, weightlifting and athletic performance shoes with lateral stability.',
  },
];
...
const BRAND_CATALOGUE = [
  ...
  },
  {
    ...
    models: [
      ...
      {
        name: 'Euro Hiker Waterproof Boot',
        material: 'Genuine Leather',
        price: 140.0,
        gender: 'unisex',
        tags: [
      ...
      {
        name: 'Bradstreet Leather Oxford',
        material: 'Genuine Leather',
        price: 120.0,
        gender: 'men',
        tags: [
  ...
  },
  {
    ...
    models: [
      {
        name: 'Clifton 9 Maximum Cushion',
        material: 'Knit Upper',
        price: 145.0,
        gender: 'unisex',
        tags: [
  ...
  },
  {
    ...
    models: [
      ...
      {
        name: 'Adrenaline GTS 23 Guardrail',
        material: 'Synthetic Mesh',
        price: 140.0,
        gender: 'women',
        tags: [
          ...
          'brooks',
          'women',
          'guide rails holistic support',
          'overpronation',
          'smooth ride',
        ],
      },
      {
        name: 'Cascadia 17 Mountain Trail',
        material: 'Synthetic Mesh',
        price: 140.0,
        gender: 'men',
        tags: [
  ...
  },
  {
    ...
    models: [
      {
        name: 'Speedcross 6 GORE-TEX Trail',
        material: 'Synthetic Mesh',
        price: 160.0,
        gender: 'unisex',
        tags: [
      ...
      {
        name: 'XT-6 Advanced Street Trail',
        material: 'Synthetic Mesh',
        price: 200.0,
        gender: 'unisex',
        tags: [
      ...
      {
        name: 'Ultra Glide 2 Max Comfort',
        material: 'Synthetic Mesh',
        price: 150.0,
        gender: 'men',
        tags: [
  ...
  },
  {
    ...
    models: [
      {
        name: 'Curry 11 Championship Hoops',
        material: 'Knit Upper',
        price: 160.0,
        gender: 'men',
        tags: [
          ...
          'men',
          'steph curry',
          'ua flow dual density',
          'warp technology',
          'court traction',
        ],
      },
      {
        name: 'HOVR Sonic 6 Light & Fast',
        material: 'Synthetic Mesh',
        price: 110.0,
        gender: 'unisex',
        tags: [
          ...
          'under armour',
          'unisex',
          'ua hovr zero gravity',
          'energy web',
          'breathable road',
        ],
      },
      {
        name: 'Tribase Reign 6 Crossfit',
        material: 'Synthetic Mesh',
        price: 130.0,
        gender: 'men',
        tags: [
    ...
    models: [
      ...
      },
      {
        ...
        tags: [
          ...
          'pwrrun+ full length',
          'plush collar',
          'daily marathon mile eater',
        ],
      },
      {
        name: 'Kinvara 14 Lightweight Racer',
        material: 'Synthetic Mesh',
        price: 120.0,
        gender: 'women',
        tags: [
...
function generateComprehensiveProducts() {
  ...
  for (const group of BRAND_CATALOGUE) {
    ...
    for (let mIdx = 0; mIdx < models.length; mIdx++) {
      ...
      const compareAt =
        skuCounter % 3 === 0 ? Number((price * 1.25).toFixed(2)) : null;
      ...
      const primaryImageIndex =
        (skuCounter * 3 + mIdx) % UNIQUE_SHOE_IMAGES.length;
      const secondaryImageIndex =
        (primaryImageIndex + 5) % UNIQUE_SHOE_IMAGES.length;
      ...
      const colorImages = primaryColors.map((colorName, cIdx) => {
        const fallbackGallery =
          COLOR_IMAGE_MAP[colorName] || COLOR_IMAGE_MAP.Black;
        const colorSpecificImg =
          fallbackGallery[(skuCounter + cIdx) % fallbackGallery.length] ||
          uniqueMainImg;
        return {
          color: colorName,
  ...
  for (let i = baseCount; i < 212; i++) {
    ...
    const name =
      `${parent.brand} ${parent.name.split(' ').slice(1).join(' ')} ${pfx} ${i + 1}`.toUpperCase();
    ...
    const colors =
      i % 2 === 0 ? ['Black', 'White', 'Blue'] : ['Grey', 'Red', 'Tan'];
    const uniqueImg =
      UNIQUE_SHOE_IMAGES[(i * 7 + 3) % UNIQUE_SHOE_IMAGES.length];
    const uniqueSecondary =
      UNIQUE_SHOE_IMAGES[(i * 7 + 9) % UNIQUE_SHOE_IMAGES.length];
...
export async function seedDatabase() {
  ...
  const [dpCheck] = await pool.query(
  ...
  for (let i = 0; i < customerNames.length; i++) {
    ...
    const [uCheck] = await pool.query('SELECT id FROM users WHERE email = ?', [
      email,
    ]);
    if (uCheck.length) {
  ...
  for (const p of products) {
    ...
    const categoryId =
      categoryMap.get(p.category) || categoryMap.get('Sneakers');
    const [prodResult] = await pool.query(
      ...
      [
        ...
        p.price,
        p.compareAt,
        Number((p.price * 0.45).toFixed(2)),
        p.featured ? 1 : 0,
        JSON.stringify(p.images),
        JSON.stringify(p.colorImages),
        JSON.stringify(p.tags),
        (4.4 + (createdCount % 6) * 0.1).toFixed(1),
        30 + ((createdCount * 7) % 350),
      ]
    );
  ...
  logger.info(
    { totalProductsSeeded: createdCount },
    '210+ Products & Color Variants created successfully.'
  );

  // 5. Seed 120 realistic orders
  const [allProds] = await pool.query(
    'SELECT id, public_id, name, slug, sku, price, images FROM products LIMIT 60'
  );
  const statuses = [
    'delivered',
    'shipped',
  ...
  if (allProds.length > 0 && userIds.length > 0) {
    ...
    if (existingOrders[0].count < 100) {
      for (let i = 0; i < 120; i++) {
        ...
        const custEmail =
          `customer${(i % customerNames.length) + 1}@example.com`.toLowerCase();
  ...
  if (
    Number(counts.products) < required.products ||
    Number(counts.users) < required.users ||
    Number(counts.orders) < required.orders
  ) {
    throw new Error(
      `Seed verification failed: expected at least ${required.products} products, ${required.users} users and ${required.orders} orders; got ${counts.products}/${counts.users}/${counts.orders}.`
    );
  }

  logger.info(
    { ...counts, uniqueMainImages: imageCounts.unique_main_images },
