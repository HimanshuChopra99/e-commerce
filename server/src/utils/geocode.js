...
export function buildAddressQueries(address) {
  ...
  return Array.from(
    new Set([fullQuery, noZipQuery, simpleQuery].filter(Boolean))
  );
}
...
export async function geocodeAddress(address) {
  ...
  try {
    ...
    logger.debug(
      { address },
      'Geocode returned no matches after all fallbacks'
    );
    return null;
  } catch (error) {
    logger.warn(
      { err: error.message },
      'Geocode lookup errored — continuing without coordinates'
    );
    return null;
  } finally {
    clearTimeout(timer);
