import { z } from 'zod'
import {
  GENDERS, SIZES, PRODUCT_STATUS, CATEGORY_COLORS,
...
export const productQuerySchema = z.object({
  ...
  sort: z
    .enum([
      ...
      'popular', 'rating', 'name_asc', 'name_desc',
...
export const createProductSchema = z
  .object({
    ...
    gender: z.enum(GENDERS, { errorMap: () => ({ message: 'Pick who this is for.' }) }),
  ...
  .refine(
    (d) => !d.compareAtPrice || d.compareAtPrice === 0 || d.compareAtPrice > d.price,
    {
...
export const updateProductSchema = z
  .object({
    ...
    sku: z.string().trim().min(3).max(60).regex(/^[A-Za-z0-9_-]+$/).optional(),
...
export const bulkStatusSchema = z.object({
  productIds: z.array(z.string().trim().min(1)).min(1, 'Select at least one product.').max(200),
