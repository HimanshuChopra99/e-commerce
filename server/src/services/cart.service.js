...
export async function resolveCartUserId(userId) {
  ...
  if (typeof userId === 'number' || /^\d+$/.test(String(userId))) return Number(userId)
...
export async function addItem(userId, { variantId, quantity }) {
  ...
  await cartModel.mergeItem(rid, variant.internalId, variant.publicId, quantity)
  return refresh(rid)
}
  ...
  if (variant) {
    ...
    try {
      if (isDatabaseConnected()) {
        ...
        await pool.query('DELETE ci FROM cart_items ci JOIN product_variants v ON v.id = ci.variant_id WHERE ci.user_id = ? AND v.public_id = ?', [rid, String(variantPublicId)])
      }
