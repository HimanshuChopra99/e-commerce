...
export async function createPaymentIntent(orderPublicId, requester) {
  ...
  if (!isAdmin && !owns && !isGuest) throw ApiError.notFound('Order not found.')
  ...
  try {
    ...
    logger.error({ err, orderNumber: order.orderNumber }, 'Stripe intent creation failed')
    throw new ApiError(502, 'PAYMENT_PROVIDER_ERROR', 'Could not start the payment. Please try again.')
  }
}
...
export async function getPaymentStatus(orderPublicId, requester) {
  ...
  const isAdmin = requester?.role === 'admin'
  const owns = order.customerId && requester?.publicId === order.customerId
  if (!isAdmin && !owns && order.customerId) throw ApiError.notFound('Order not found.')
...
export async function handlePaymentSucceeded(intent) {
  ...
  await withTransaction(async (conn) => {
    ...
    if (row.payment_status === 'paid') {
      logger.info({ orderNumber: row.order_number }, 'order already paid, skipping')
      return
    }
    ...
    for (const item of items) {
      ...
      await variantModel.commitReservation(item.variant_id, item.quantity, conn)
      if (item.product_id) {
        await productModel.incrementUnitsSold(item.product_id, item.quantity, conn)
...
export async function handlePaymentFailed(intent) {
  ...
  await withTransaction(async (conn) => {
    ...
    logger.warn(
      { orderNumber: row.order_number, reason: intent.last_payment_error?.message },
      'payment failed'
    )
  })
...
export async function handleRefund(charge) {
  ...
  await withTransaction(async (conn) => {
    ...
    await orderModel.recordRefund(row.id, fromCents(refundedCents), fully, conn)
    logger.info(
      { orderNumber: row.order_number, refunded: centsToNumber(refundedCents), fully },
      'refund recorded'
    )
  })
  if (customerId) await deleteCachedPattern(`customer:${customerId}:orders:*`)
...
export async function refundOrder(orderPublicId, { amount, reason, restock = false }) {
  ...
  if (!intentId) throw ApiError.badRequest('No Stripe payment is attached to this order.')

  const alreadyCents = toCents(order.refundedAmount)
  const maxCents = toCents(order.total) - alreadyCents
  if (maxCents <= 0) throw ApiError.badRequest('This order is already fully refunded.')
  ...
  try {
    ...
    if (err.type?.startsWith('Stripe')) {
      logger.error({ err, orderNumber: order.orderNumber }, 'Stripe refund failed')
