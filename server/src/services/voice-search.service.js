...
function buildLookupIndexes() {
  ...
  for (const p of productCache) {
    ...
    if (p.category) {
      const slug = (
        p.category.slug || (typeof p.category === 'string' ? p.category : '')
      ).trim();
      const name = (
        p.category.name || (typeof p.category === 'string' ? p.category : '')
      ).trim();
      if (slug)
        _categoryMap.set(slug.toLowerCase(), { slug, name: name || slug });
      if (name)
        _categoryMap.set(name.toLowerCase(), {
  ...
  logger.info(
    {
      brands: _brandMap.size,
      categories: _categoryMap.size,
      colors: _colorMap.size,
      materials: _materialMap.size,
    },
...
export function extractBrand(rawText) {
  ...
  if (m)
    return (
      _brandMap.get(m[1].toLowerCase()) ||
      _brandMap.get(m[0].toLowerCase()) ||
      null
    );
...
export function extractCategory(rawText) {
  ...
  if (m)
    return (
      _categoryMap.get(m[1].toLowerCase()) ||
      _categoryMap.get(m[0].toLowerCase()) ||
      null
    );
  const words = text
    .split(/[^a-z0-9]+/)
  ...
  for (const w of words) {
    for (const [, def] of _categoryMap) {
      if (
        stringSimilarity(w, def.slug.toLowerCase()) >= 0.75 ||
        stringSimilarity(w, def.name.toLowerCase()) >= 0.75
      )
        return def;
    }
  }
...
export function extractGender(rawText) {
  ...
  if (
    /\b(women|woman|womens|women's|female|females|lady|ladies|girl|girls|wmn|wman|wmen|womans|womin)\b/i.test(
      t
    )
  )
    return 'women';
  if (
    /\b(men|man|mens|men's|male|males|guy|guys|boy|boys|gentleman|gentlemen|mn)\b/i.test(
      t
    )
  )
    return 'men';
  if (
    /\b(unisex|unisexual|all\s+gender|all-gender|gender\s+neutral|neutral|for\s+both|universal|everyone)\b/i.test(
      t
    )
  )
    return 'unisex';
  if (/\b(kids|kid|child|children|youth|toddler|junior)\b/i.test(t))
    return 'kids';
  return null;
}
...
export function extractColor(rawText) {
  ...
  if (_colorRegex) {
    ...
    if (m)
      return (
        _colorMap.get(m[1].toLowerCase()) ||
        _colorMap.get(m[0].toLowerCase()) ||
        null
      );
  }
  const words = text
...
export function extractMaterial(rawText) {
  ...
  if (m)
    return (
      _materialMap.get(m[1].toLowerCase()) ||
      _materialMap.get(m[0].toLowerCase()) ||
      null
    );
  const words = text
    .split(/[^a-z0-9]+/)
...
export function extractSize(rawText) {
  ...
  const withKeyword =
    text.match(/\b(?:size|eu|us)\s*:?\s*(\d+(?:\.\d+)?)\b/i) ||
  ...
  const WORD_TO_NUM = {
    ...
    'forty seven': '47',
    'forty-seven': '47',
    'thirty eight point five': '38.5',
    'thirty nine point five': '39.5',
    'forty point five': '40.5',
    'forty one point five': '41.5',
    'forty two point five': '42.5',
    'forty three point five': '43.5',
    'forty four point five': '44.5',
  ...
  const shoeSizes = bareMatches.filter((m) => {
...
export function extractPriceIntent(rawText) {
  ...
  const under = text.match(
    /(?:under|below|less\s+than|max|up\s+to|cheaper\s+than|within|<=?)\s+\$?(\d+(?:\.\d+)?)/i
  );
  const above = text.match(
    /(?:above|over|more\s+than|greater\s+than|at\s+least|min|minimum|starting\s+from|>=?)\s+\$?(\d+(?:\.\d+)?)/i
  );
  const around = text.match(
    /(?:around|about|approx|approximately)\s+\$?(\d+(?:\.\d+)?)/i
  );
  ...
  if (between) {
    ...
    const v = Number(around[1]);
    result.minPrice = Math.max(0, Math.round(v * 0.8));
    result.maxPrice = Math.round(v * 1.2);
  }

  if (
    /\b(cheap|cheapest|affordable|budget|low\s+price|economy|discount|sale)\b/i.test(
      text
    )
  )
    result.sort = 'price_asc';
  else if (
    /\b(expensive|priciest|premium|luxury|high\s+end|highest\s+price)\b/i.test(
      text
    )
  )
    result.sort = 'price_desc';
  else if (
    /\b(new|newest|latest|recent|fresh|new\s+arrivals|just\s+dropped)\b/i.test(
      text
    )
  )
    result.sort = 'newest';
  else if (
    /\b(popular|best\s+seller|best\s+selling|top\s+seller|trending|hot)\b/i.test(
      text
    )
  )
    result.sort = 'popular';
  else if (
    /\b(top\s+rated|best\s+rated|highest\s+rating|best\s+reviews|5\s+star|rating)\b/i.test(
      text
    )
  )
    result.sort = 'rating';
...
export function isExplicitOpenIntent(rawText) {
  ...
  return /\b(open|view|go\s+to|details\s+of|take\s+me\s+to|show\s+details|select)\b/i.test(
    String(rawText)
  );
}
...
export function calculateNameSimilarity(queryText, product) {
  ...
  const qWithoutBrand =
    brand && cleanQ.includes(brand)
      ...
      : cleanQ;
  const strippedSim =
    qWithoutBrand.length >= 3 ? stringSimilarity(qWithoutBrand, modelName) : 0;

  let substringScore = 0;
  if (qWithoutBrand.length >= 4 && modelName.includes(qWithoutBrand)) {
    substringScore = Math.min(
      0.95,
      qWithoutBrand.length / modelName.length + 0.35
    );
  }

  const qTokens = cleanQ
    .split(' ')
    .filter((t) => t.length > 1 && !STOP_WORDS.has(t) && t !== brand);
  const targetTokens = modelName
    .split(' ')
  ...
  for (const qt of qTokens) {
    ...
    for (const tt of targetTokens) {
      const s = stringSimilarity(qt, tt);
      if (s > best) best = s;
    }
    if (best >= 0.75) {
      matchedTokens++;
...
export function buildProductsUrl(
  query,
  { gender, color, size, category, minPrice, maxPrice, sort } = {}
) {
...
async function loadProducts() {
  try {
    const keys = [
      ...
      'public:products:list:{"storefront":true}',
    ];
...
export async function buildIndex() {
  try {
    ...
    logger.info(
      ...
      'voice-search: Fuse index built successfully'
    );
  } catch (err) {
...
export async function search(input) {
  ...
  if (typeof input === 'string') {
    ...
    explicitMinPrice =
      input.min_price ?? input.price_min ?? input.minPrice ?? null;
    explicitMaxPrice =
      input.max_price ?? input.price_max ?? input.maxPrice ?? null;
  ...
  const categoryDef = explicitCategory
    ? _categoryMap.get(explicitCategory.toLowerCase()) || {
        slug: explicitCategory.toLowerCase().replace(/\s+/g, '-'),
        name: explicitCategory,
      }
    : extractCategory(combinedText);
  const gender = explicitGender
    ? extractGender(explicitGender)
    : extractGender(combinedText);
  const color = explicitColor
    ? _colorMap.get(explicitColor.toLowerCase()) || explicitColor
    : extractColor(combinedText);
  const size = explicitSize || extractSize(combinedText);
  const material = explicitMaterial
    ? _materialMap.get(explicitMaterial.toLowerCase()) || explicitMaterial
    : extractMaterial(combinedText);
  const priceIntent = extractPriceIntent(combinedText);
  const minPrice =
    explicitMinPrice !== null ? Number(explicitMinPrice) : priceIntent.minPrice;
  const maxPrice =
    explicitMaxPrice !== null ? Number(explicitMaxPrice) : priceIntent.maxPrice;
  ...
  for (const w of rawWords) {
    ...
    if (['best', 'top', 'cheap', 'cheapest', 'expensive', 'newest'].includes(w))
      continue;
    if (
      gender &&
      (w === gender ||
        ...
        (gender === 'unisex' && ['unisexual', 'universal'].includes(w)))
    )
      continue;
    if (
      categoryDef &&
      (w === categoryDef.slug ||
        w === (categoryDef.name || '').toLowerCase() ||
        (categoryDef.slug === 'running' &&
          ['running', 'runing', 'runner'].includes(w)) ||
        (categoryDef.slug === 'sneakers' &&
          ['sneakers', 'sneaker', 'snickers', 'kicks'].includes(w)) ||
        (categoryDef.slug === 'formal' &&
          ['formal', 'formel', 'dress'].includes(w)) ||
        (categoryDef.slug === 'boots' && ['boots', 'boot'].includes(w)) ||
        (categoryDef.slug === 'basketball' &&
          ['basketball', 'basktball', 'bball'].includes(w)))
    )
      continue;
  ...
  const modelKeywords = residualWords.filter(
  ...
  if (brandName) {
    ...
    const bm = candidatePool.filter(
      (p) => stringSimilarity(p.brand, bL) >= 0.75
    );
  ...
  if (color) {
    const cm = candidatePool.filter(
      (p) =>
        p.colors?.some(
          (c) => typeof c === 'string' && stringSimilarity(c, color) >= 0.8
        ) ||
        p.colorImages?.some(
          (ci) => ci?.color && stringSimilarity(ci.color, color) >= 0.8
        ) ||
        p.variants?.some(
          (v) => v?.color && stringSimilarity(v.color, color) >= 0.8
        )
    );
    if (cm.length) candidatePool = cm;
  }
  if (material) {
    const mm = candidatePool.filter(
      (p) => stringSimilarity(p.material || '', material) >= 0.75
    );
  ...
  const scoredItems = scoredResults.map(({ item, fuseScore }) => {
    const nameMatchScore = calculateNameSimilarity(
      cleanedTextQuery || rawQuery,
      item
    );
    return {
      item,
      nameMatchScore,
      relevanceScore: nameMatchScore * 0.6 + (1 - (fuseScore ?? 0.5)) * 0.4,
    };
  });
  ...
  if (sort === 'price_asc')
    ...
    scoredItems.sort(
      (a, b) => (b.item.featured ? 1 : 0) - (a.item.featured ? 1 : 0)
    );
  else if (sort === 'popular')
    scoredItems.sort(
      (a, b) => (b.item.unitsSold || 0) - (a.item.unitsSold || 0)
    );
  else if (sort === 'rating')
  ...
  if (!scoredItems.length) {
    ...
    return {
      ...
      navigateTo: buildProductsUrl(brandName || null, {
  ...
  const hasDistinctModelName =
    modelKeywords.length > 0 && cleanedTextQuery.length >= 3;
  const shouldOpenDirectly =
    !isSuggestion &&
    ((hasDistinctModelName && top.nameMatchScore >= 0.8) || isExplicitOpen);

  if (shouldOpenDirectly) {
    const product = top.item;
    const stockMsg = product.inStock
  ...
  const residualQuery =
    cleanedTextQuery.length >= 2 ? cleanedTextQuery : brandName || null;
  const navigateTo = buildProductsUrl(residualQuery, {
    category: categoryDef?.slug,
    gender,
    color,
    size,
