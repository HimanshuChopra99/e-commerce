...
async function executeRetellSync(state) {
  try {
    const dynamicVariables = {
      ...
      viewed_product: formatProductContext(
    ...
    logger.info(
      { callId: state.callId, dynamicVariables, shouldTriggerResponse },
      '[RetellSync] Updating dynamic variables for active call via updateLive'
    );
    ...
    logger.warn(
      { err: err.message, callId: state.callId },
      '[RetellSync] Failed to update call dynamic variables'
    );
  }
}
...
function formatPageContext(state) {
  ...
  if (p === '/checkout/payment' || p.startsWith('/checkout'))
    return 'Checkout page';
  if (p === '/products' || state.type === 'catalog')
    return 'Product Catalog page';
  if (p === '/profile' || p === '/favourites' || p === '/wishlist')
...
function formatProductContext(product, selectedColor, selectedSize) {
  ...
  if (selectedColor || product.color)
    details.push(`Selected Color: ${selectedColor || product.color}`);
  if (selectedSize || product.size)
    details.push(`Selected Size: ${selectedSize || product.size}`);
  if (product.colors?.length)
    details.push(`Available Colors: ${product.colors.join(', ')}`);
  if (product.sizes?.length)
    details.push(`Available Sizes: ${product.sizes.join(', ')}`);
  if (product.inStock !== undefined)
    details.push(`Stock: ${product.inStock ? 'In Stock' : 'Out of Stock'}`);
  return details.join(' | ');
}

function formatCartContext(cart) {
  if (!cart || !cart.items || cart.items.length === 0)
    return 'Cart is empty (0 items, $0.00)';
  const itemDescs = cart.items.map(
    (i) =>
      `${i.quantity}x ${i.name}${i.size ? ` (Size ${i.size})` : ''}${i.color ? ` (${i.color})` : ''} - $${((i.price || 0) * i.quantity).toFixed(2)}`
  );
