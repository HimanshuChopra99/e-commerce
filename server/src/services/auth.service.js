...
export async function register(input, context = {}) {
  ...
  try {
    ...
    if (err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND' || !err.code) {
      ...
      logger.info({ userId: newUser.publicId }, 'user registered (memory store)')
  ...
  if (!user) {
    ...
    if (memoryUser) {
      ...
      if (matches) {
        ...
        logger.info({ userId: memoryUser.publicId, role: memoryUser.role }, 'user signed in (memory store)')
...
export async function getProfile(userId) {
  ...
  try {
    ...
    const user = memoryUsers.find((u) => u.internalId === userId || u.id === userId)
    if (!user) throw ApiError.notFound('Account not found.')
...
export async function updateProfile(userId, patch) {
  try {
    ...
    const user = memoryStore.getUsers().find((u) => u.internalId === userId || u.id === userId)
    if (!user) throw ApiError.notFound('Account not found.')
