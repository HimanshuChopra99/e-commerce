...
function flattenColorImages(entries) {
  return [...new Set(entries.flatMap((entry) => entry.images))].slice(
    0,
    MAX_COLOR_IMAGES
  );
}
...
export async function getPublicBySlug(slug) {
  return cachedPublic(
    `${PUBLIC_CACHE_PREFIX}products:slug:${slug}`,
    CACHE_TTL.PRODUCT,
    async () => {
      ...
      const variants = await variantModel.findByProduct(
...
export async function getRelated(slug, limit = 4) {
  return cachedPublic(
    `${PUBLIC_CACHE_PREFIX}products:related:${slug}:${limit}`,
    CACHE_TTL.PRODUCT,
    async () => {
      const product = await productModel.findBySlug(slug);
      ...
      const items = await productModel.findRelated(
        category.internalId,
        product.internalId,
        limit
      );
...
export async function listFeatured(limit = 8) {
  return cachedPublic(
    `${PUBLIC_CACHE_PREFIX}products:featured:${limit}`,
    CACHE_TTL.PRODUCTS,
    async () => {
...
export async function create(rawInput) {
  ...
  if (!stocked.length)
    throw ApiError.badRequest('Add stock to at least one size.');

  const totalStock =
    stocked.reduce((sum, v) => sum + v.stock, 0) * (input.colors?.length || 1);
  ...
  if (isDatabaseConnected()) {
    try {
      const res = await withTransaction(async (conn) => {
        const productInternalId = await productModel.create(
          {
            ...
            status:
              totalStock === 0 && input.status === 'active'
                ? 'out_of_stock'
                : input.status,
        ...
        const created = await productModel.findByInternalId(
          productInternalId,
          conn
        );
...
export async function update(productPublicId, rawInput) {
  ...
  if (input.colorImages?.length && !input.colors) {
    const existingVariants = await variantModel.findByProduct(
      product.internalId || product.id
    );
    const productColors = [
    ...
    const assigned = new Set(
      input.colorImages.map((entry) => entry.color.toLocaleLowerCase())
    );
    const invalid = input.colorImages.find(
      (entry) =>
        !productColors.some(
          (color) =>
            color.toLocaleLowerCase() === entry.color.toLocaleLowerCase()
        )
    );
    const missing = productColors.find(
      (color) => !assigned.has(color.toLocaleLowerCase())
    );
    if (invalid)
      throw ApiError.badRequest(
        `Colour "${invalid.color}" is not available on this product.`
      );
    if (missing)
      throw ApiError.badRequest(`Add at least one image for ${missing}.`);
  }
      ...
      const res = await withTransaction(async (conn) => {
        ...
        if (input.variants || input.colors) {
          ...
          const colors = input.colors ?? [
            ...new Set(existing.map((v) => v.color)),
          ];
        ...
        const updated = await productModel.findByInternalId(
          product.internalId,
          conn
        );
  ...
  if (input.variants || input.colors) {
    ...
    const colors = input.colors ?? [
      ...new Set(existing.map((variant) => variant.color)),
    ];
    const variants = input.variants ?? [
      ...new Map(
    ...
    patch.totalStock = patch.variants.reduce(
  ...
  if (isDatabaseConnected()) {
    try {
      ...
      return await withTransaction(async (conn) => {
        ...
        await productModel.recalcStock(product.internalId, conn);
        const updated = await variantModel
          .findByProduct(product.internalId)
          .then((rows) =>
  ...
  if (product.variants) {
    for (const change of variants) {
      const v = product.variants.find(
        (v) =>
          v.size === change.size &&
          (change.color ? v.color === change.color : true)
      );
      if (v) {
...
export async function remove(productPublicId) {
  ...
  if (variants.some((v) => v.reserved > 0)) {
    throw ApiError.conflict(
      'This product has stock reserved by an open checkout.'
    );
  }
...
export async function addImages(productPublicId, urls, color = null) {
  ...
  if (color) {
    ...
    const actualColor = [
      ...
      (value) =>
        value.toLocaleLowerCase() === String(color).trim().toLocaleLowerCase()
    );
    if (!actualColor)
      throw ApiError.badRequest(
        'That colour is not available on this product.'
      );

    const current = colorImages.find(
      (entry) =>
        entry.color.toLocaleLowerCase() === actualColor.toLocaleLowerCase()
    );
    if (current) {
      ...
      colorImages.push({
        color: actualColor,
        images: [...new Set(urls)].slice(0, 8),
      });
    }
    images = flattenColorImages(colorImages);
  } else {
    images = [...new Set([...(product.images ?? []), ...urls])].slice(
      0,
      MAX_COLOR_IMAGES
    );
  }
...
export async function removeImage(productPublicId, url, color = null) {
  ...
  if (color) {
    ...
    colorImages = colorImages
      .map((entry) => ({
        ...entry,
        images: entry.images.filter((image) => image !== url),
      }))
      .filter((entry) => entry.images.length > 0);
  }
...
export async function getLowStock(limit = 50) {
  const items = await productModel.findLowStock(
    env.business.lowStockThreshold,
    limit
  );
