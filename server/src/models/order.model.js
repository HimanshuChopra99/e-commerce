...
export function mapOrder(row) {
  ...
  return {
    ...
    itemCount:
      row.item_count !== undefined ? Number(row.item_count) : undefined,
    shippingAddress: {
...
export function mapOrderMemory(mem) {
  ...
  return {
    ...
    customerEmail:
      mem.customer?.email || mem.customerEmail || 'customer@example.com',
...
export async function findByPublicId(publicId) {
  if (isDatabaseConnected()) {
    try {
      const cleanNumber = String(publicId).startsWith('#')
        ? String(publicId)
        : `#${publicId}`;
...
export async function findByInternalId(id, conn = pool) {
  ...
  const mem = memoryStore
    .getOrders()
    .find((o) => o.internalId === id || o.id === id || o.publicId === id);
  return mem ? mapOrderMemory(mem) : null;
}
...
export async function findByOrderNumber(orderNumber) {
  if (isDatabaseConnected()) {
    try {
      const row = await queryOne(`${BASE} WHERE o.order_number = ? LIMIT 1`, [
        orderNumber,
      ]);
      if (row) return mapOrder(row);
    } catch {}
  ...
  if (isDatabaseConnected()) {
    try {
      const [rows] = await conn.query(
        `${BASE} WHERE o.stripe_payment_intent_id = ? LIMIT 1`,
        [intentId]
      );
      if (rows && rows[0]) return mapOrder(rows[0]);
    } catch {}
...
export async function findItems(orderInternalId, conn = pool) {
  ...
  const mem = memoryStore
    ...
    .find(
      (o) =>
        o.internalId === orderInternalId ||
        o.id === orderInternalId ||
        o.publicId === orderInternalId
    );
...
export async function findAll(filters = {}) {
  ...
  if (userId !== undefined && userId !== null) {
    ...
    const user = memoryStore
      ...
      .find(
        (u) =>
          String(u.internalId) === userIdStr ||
          u.id === userIdStr ||
          u.publicId === userIdStr
      );
    ...
    list = list.filter((o) => {
      ...
      return (
        (oCustId &&
          (oCustId === String(userPublicId) ||
            oCustId === String(userInternalId))) ||
        (oUserId &&
          (oUserId === String(userPublicId) ||
            oUserId === String(userInternalId))) ||
  ...
  return {
    items: list.slice(offset, offset + limit).map(mapOrderMemory),
    total: list.length,
  };
}
    ...
    try {
      const [result] = await conn.query(
        ...
        [
          ...
          data.shippingAddress.lat ?? null,
          data.shippingAddress.lng ?? null,
...
export async function updateStatus(
  ...
  if (isDatabaseConnected()) {
    try {
      ...
      if (extra.courier !== undefined) {
        sets.push('courier = ?');
        params.push(extra.courier);
      }
  ...
  const mem = memoryStore
    ...
    .find(
      (o) =>
        String(o.internalId) === String(orderInternalId) ||
        o.id === orderInternalId ||
        o.publicId === orderInternalId
    );
  if (mem) {
...
export async function updateNote(orderInternalId, adminNote) {
  if (isDatabaseConnected()) {
    try {
      await query('UPDATE orders SET admin_note = ? WHERE id = ?', [
        adminNote,
        orderInternalId,
      ]);
    } catch {}
...
export async function getRecentOrders(limit = 6) {
  if (isDatabaseConnected()) {
    try {
      const rows = await query(`${BASE} ORDER BY o.placed_at DESC LIMIT ?`, [
        limit,
      ]);
...
export async function getProductSalesStats(productInternalId) {
  ...
  return { unitsSold: 25, revenue: 2450.0 };
}
