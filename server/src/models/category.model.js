  ...
  if (isDatabaseConnected()) {
    try {
      const row = await queryOne(
        `${WITH_COUNT} WHERE c.public_id = ? LIMIT 1`,
        [publicId]
      );
...
export async function findBySlug(slug) {
  if (isDatabaseConnected()) {
    try {
      const row = await queryOne(`${WITH_COUNT} WHERE c.slug = ? LIMIT 1`, [
        slug,
      ]);
      if (row) return mapCategory(row);
    } catch {}
  ...
  if (isDatabaseConnected()) {
    try {
      const row = ignoreInternalId
        ? await queryOne(
            'SELECT 1 AS x FROM categories WHERE slug = ? AND id <> ? LIMIT 1',
            [slug, ignoreInternalId]
          )
        : await queryOne(
            'SELECT 1 AS x FROM categories WHERE slug = ? LIMIT 1',
            [slug]
          );
...
export async function create(data, conn = pool) {
  if (isDatabaseConnected()) {
    try {
      ...
      const row = await queryOne(`${WITH_COUNT} WHERE c.id = ?`, [
        result.insertId,
      ]);
      if (row) return mapCategory(row);
    } catch {}
...
export async function update(internalId, patch) {
  if (isDatabaseConnected()) {
    try {
      ...
      if (!sets.length) {
        return mapCategory(
          await queryOne(`${WITH_COUNT} WHERE c.id = ?`, [internalId])
        );
      }
      ...
      const updated = await queryOne(`${WITH_COUNT} WHERE c.id = ?`, [
        internalId,
      ]);
