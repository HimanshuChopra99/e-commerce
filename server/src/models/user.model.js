  ...
  if (isDatabaseConnected()) {
    try {
      const row = await queryOne(
        `SELECT ${SAFE_COLUMNS} FROM users WHERE public_id = ? LIMIT 1`,
        [publicId]
      );
...
export async function findById(id) {
  ...
  const mem = memoryStore
    .getUsers()
    .find((u) => u.internalId === id || u.id === id || u.publicId === id);
  return mem ? mapMemoryUser(mem) : null;
}
...
export async function update(id, patch) {
  ...
  if (patch.preferredSize !== undefined)
    mem.preferredSize = patch.preferredSize;
  if (patch.marketingOptIn !== undefined)
    mem.marketingOptIn = Boolean(patch.marketingOptIn);
...
export async function markEmailVerified(id) {
  if (isDatabaseConnected()) {
    try {
      await query('UPDATE users SET email_verified_at = NOW() WHERE id = ?', [
        id,
      ]);
      return;
    } catch {}
...
export async function getCustomerStats(userId) {
  ...
  const mem = memoryStore
    .getUsers()
    .find(
      (u) => u.internalId === userId || u.id === userId || u.publicId === userId
    );
  return {
    totalOrders: mem?.totalOrders ?? 0,
    totalSpent: mem?.totalSpent ?? 0,
...
export async function countNewSince(date) {
  ...
  return memoryStore
    .getUsers()
    .filter(
      (u) => u.role === 'customer' && new Date(u.createdAt) >= new Date(date)
    ).length;
}
...
export async function countBetween(from, to) {
  ...
  return memoryStore
    ...
    .filter(
      (u) =>
        u.role === 'customer' &&
        new Date(u.createdAt) >= new Date(from) &&
        new Date(u.createdAt) < new Date(to)
    ).length;
}
