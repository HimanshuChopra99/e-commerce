...
export function toPublicVariant(variant) {
  ...
  const available =
    variant.available !== undefined && variant.stock === undefined
...
export async function findByProduct(
  ...
  const product = memoryStore.products.find(
    (p) =>
      p.internalId === productInternalId ||
      p.id === productInternalId ||
      p.publicId === productInternalId
  );
...
export async function findByPublicId(publicId) {
  if (isDatabaseConnected()) {
    try {
      const row = await queryOne(
        'SELECT * FROM product_variants WHERE public_id = ? LIMIT 1',
        [publicId]
      );
  ...
  for (const prod of memoryStore.products) {
    const v = prod.variants?.find(
      (v) => v.publicId === publicId || v.id === publicId
    );
    if (v) {
      return {
        id: v.publicId || v.id,
  ...
  if (isDatabaseConnected()) {
    try {
      const row = ignoreInternalId
        ? await queryOne(
            'SELECT 1 AS x FROM product_variants WHERE sku = ? AND id <> ? LIMIT 1',
            [sku, ignoreInternalId]
          )
        : await queryOne(
            'SELECT 1 AS x FROM product_variants WHERE sku = ? LIMIT 1',
            [sku]
          );
      if (row) return true;
    } catch {}
...
export async function deleteById(internalId, conn = pool) {
  if (isDatabaseConnected()) {
    try {
      await conn.query('DELETE FROM product_variants WHERE id = ?', [
        internalId,
      ]);
    } catch {}
  }
