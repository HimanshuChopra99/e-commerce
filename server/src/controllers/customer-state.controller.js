...
export const removeCartItem = asyncHandler(async (req, res) => {
  ok(res, {
    items: await cartService.removeItem(req.user.id, req.params.variantId),
  });
});
...
export const addFavourite = asyncHandler(async (req, res) => {
  ok(res, {
    products: await favouriteService.add(req.user.id, req.params.productId),
  });
});

export const removeFavourite = asyncHandler(async (req, res) => {
  ok(res, {
    products: await favouriteService.remove(req.user.id, req.params.productId),
  });
});

export const syncFavourites = asyncHandler(async (req, res) => {
  ok(res, {
    products: await favouriteService.sync(req.user.id, req.body.productIds),
  });
});
