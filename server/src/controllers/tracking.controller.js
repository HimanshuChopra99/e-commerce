...
export const savePing = asyncHandler(async (req, res) => {
  ...
  const distanceMeters =
    Number.isFinite(destination.lat) && Number.isFinite(destination.lng)
      ? Math.round(haversineMeters(ping.lat, ping.lng, destination.lat, destination.lng))
      : null
  ...
  if (io) {
    ...
    logger.debug({ trackingNumber }, 'Socket.io not initialised — ping stored without broadcast')
  }
