...
export const handleFunction = async (req, res) => {
  try {
    const function_name =
      req.body.function_name || req.body.name || req.body.func_name;
    const function_call_id =
      req.body.function_call_id ||
    ...
    const args =
      req.body.args || req.body.parameters || req.body.arguments || {};
    const userId =
      req.body.metadata?.userId ||
      req.body.call?.metadata?.userId ||
      req.body.userId ||
      'guest';
    ...
    logger.error(
      { err: err.message },
      '[RetellController] handleFunction error'
    );
    return res.status(200).json({
      function_call_id:
        req.body?.function_call_id ||
