...
export const handleFunction = async (req, res) => {
  try {
    logger.info({ body: req.body }, '[Retell] function call received raw body')

    // Flexibly support Retell Custom Tool payload (req.body.name) and Webhook payload (req.body.function_name)
    ...
    if (!function_name) {
      logger.warn({ body: req.body }, '[Retell] missing function_name in request body')
    ...
    logger.info({ function_name, function_call_id, args, userId }, '[Retell] dispatching parsed function call')

    const result = await dispatch(function_name, args, userId)

    // Retell expects a response object containing function_call_id and result string
    return res.status(200).json({
      function_call_id: function_call_id || 'call_id',
      result: typeof result === 'string' ? result : JSON.stringify(result),
    })
  } catch (err) {
    logger.error({ err: err.message }, '[Retell] handleFunction error')
    return res.status(200).json({
