...
export const updateVariants = asyncHandler(async (req, res) => {
  ok(
    res,
    await productService.updateVariantStock(req.params.id, req.body.variants)
  );
});

export const bulkStatus = asyncHandler(async (req, res) => {
  const updated = await productService.bulkStatus(
    req.body.productIds,
    req.body.status
  );
  ok(res, { updated });
...
export const uploadImageAssets = asyncHandler(async (req, res) => {
  ...
  if (!urls.length)
    throw ApiError.badRequest('Select at least one image to upload.');
...
export const deleteImage = asyncHandler(async (req, res) => {
  ...
  ok(
    ...
    await productService.removeImage(
      req.params.id,
      url,
      req.query.color ?? req.body?.color
    )
  );
});
...
export const exportCsv = asyncHandler(async (req, res) => {
  ...
  const csv = toCsv(items, [
    ...
    {
      header: 'Created',
      value: (p) => p.createdAt?.toISOString?.() ?? p.createdAt,
    },
  ]);
