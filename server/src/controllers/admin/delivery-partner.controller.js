...
export const updateStatus = asyncHandler(async (req, res) => {
  const partner = await requirePartner(req.params.id);
  const updated = await dpModel.update(partner.internalId, {
...
export const listOrders = asyncHandler(async (req, res) => {
  const { page, limit, offset } = getPagination(req.query);
  const partner = await requirePartner(req.params.id);
  const { items, total } = await dpModel.findOrders(partner.internalId, {
    limit,
    offset,
  });
  paginated(res, items, buildMeta({ page, limit, total }));
...
export const exportCsv = asyncHandler(async (req, res) => {
  ...
  res.setHeader(
    'Content-Disposition',
    'attachment; filename="delivery-partners.csv"'
  );
  res.send(csv);
});
