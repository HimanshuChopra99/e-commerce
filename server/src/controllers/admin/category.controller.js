...
export const listProducts = asyncHandler(async (req, res) => {
  ...
  const { category, items, total } = await categoryService.listProducts(req.params.id, {
    limit,
    offset,
    sort: req.query.sort,
  })
...
export const assignProducts = asyncHandler(async (req, res) => {
  ok(res, await categoryService.assignProducts(req.params.id, req.body.productIds))
})
