...
async function main() {
  ...
  if (!process.env.REDIS_URL && env.isProd) {
    logger.warn(
      'WARNING: REDIS_URL not set — rate limits are per-process. Do not run multiple instances without Redis.'
    );
  }
  ...
  buildIndex().catch((err) =>
    logger.warn({ err: err.message }, 'voice index warm-up failed')
  );
