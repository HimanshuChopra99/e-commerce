...
async function runTests() {
  ...
  console.log(
    '--- Test Suite 1: Suggestions / Recommendations (Must show list on screen) ---'
  );
  const s1 = await voiceSearch.search('suggest some shoes for running');
  assert(
    s1.type === 'multiple',
    'suggest some shoes for running -> type: multiple'
  );
  assert(
    s1.navigateTo.includes('category=running'),
    'suggest running -> category=running in URL'
  );

  const s2 = await voiceSearch.search('recommend casual shoes for women');
  assert(
    s2.type === 'multiple',
    'recommend casual shoes for women -> type: multiple'
  );
  assert(
    s2.navigateTo.includes('gender=women'),
    'recommend women -> gender=women in URL'
  );

  const s3 = await voiceSearch.search('show me cheap sneakers');
  assert(s3.type === 'multiple', 'show me cheap sneakers -> type: multiple');
  assert(
    s3.navigateTo.includes('sort=price_asc'),
    'cheap sneakers -> sort=price_asc in URL'
  );

  const s4 = await voiceSearch.search('what do you suggest for basketball');
  assert(
    s4.type === 'multiple',
    'what do you suggest for basketball -> type: multiple'
  );
  assert(
    s4.navigateTo.includes('category=basketball'),
    'suggest basketball -> category=basketball'
  );

  const s5 = await voiceSearch.search('show me unisexual shoes');
  assert(s5.type === 'multiple', 'show me unisexual shoes -> type: multiple');
  assert(
    s5.navigateTo.includes('gender=unisex'),
    'unisexual -> gender=unisex in URL'
  );
  ...
  assert(
    m1.navigateTo.includes('category=running'),
    'nikey runing -> category=running'
  );

  const m2 = await voiceSearch.search('addidas snickers for womans');
  assert(
    m2.type === 'multiple',
    'addidas snickers for womans -> type: multiple'
  );
  assert(
    m2.navigateTo.includes('category=sneakers') &&
      m2.navigateTo.includes('gender=women'),
    'addidas snickers womans -> sneakers + women'
  );

  const m3 = await voiceSearch.search('jorden basktball in blak');
  assert(m3.type === 'multiple', 'jorden basktball in blak -> type: multiple');
  assert(
    m3.navigateTo.includes('category=basketball') &&
      m3.navigateTo.includes('color=Black'),
    'jorden basktball blak -> basketball + Black'
  );

  const m4 = await voiceSearch.search('formel lether shoes for mn');
  assert(
    m4.type === 'multiple',
    'formel lether shoes for mn -> type: multiple'
  );
  assert(
    m4.navigateTo.includes('category=formal') &&
      m4.navigateTo.includes('gender=men'),
    'formel lether mn -> formal + men'
  );
  ...
  if (sampleProduct) {
    ...
    assert(
      p1.type === 'exact',
      `Exact product name "${sampleProduct.name}" -> type: exact`
    );
    assert(
      p1.product?.slug === sampleProduct.slug,
      `Returns correct slug: ${sampleProduct.slug}`
    );
    ...
    assert(
      p2.type === 'exact',
      `Typo in product name "${typoName}" -> type: exact`
    );
  }
  ...
  assert(
    d1.success === true,
    'dispatch search_product with structured filters -> success'
  );
  assert(
    d1.total !== undefined || d1.product !== undefined,
  ...
  assert(
    d2.navigateTo?.includes('category=sneakers'),
    'suggest_product navigates to sneakers list'
  );
