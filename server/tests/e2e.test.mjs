...
section('HEALTH & ROOT')
{
  ...
  check('X-Request-Id header present', Boolean(health.headers.get('x-request-id')))

  const root = await api('/')
  check('GET / returns api info', root.status === 200 && root.body.data.name === 'Kick API')

  const missing = await api('/api/does-not-exist')
  check('unknown route returns 404 envelope',
...
let customerEmail = `test.${rand}@example.com`
{
  const bad = await api('/api/auth/register', {
    method: 'POST',
    body: { firstName: 'A', lastName: 'B', email: 'not-an-email', password: 'short' },
  })
  check('register rejects bad input with 422',
  ...
  const weak = await api('/api/auth/register', {
    method: 'POST',
    body: { firstName: 'A', lastName: 'B', email: `w.${rand}@x.com`, password: 'alllowercase' },
  })
  ...
  const created = await api('/api/auth/register', {
    method: 'POST',
    body: {
      firstName: 'Test', lastName: 'Buyer', email: customerEmail,
      password: 'Password123', phone: '+91 90000 00001', marketingOptIn: true,
    },
  })
  check('register succeeds', created.status === 201, `status ${created.status}`)
  check('register returns an access token', Boolean(created.body?.data?.accessToken))
  check('register defaults role to customer', created.body?.data?.user?.role === 'customer')
  check('register never returns a password hash',
    !JSON.stringify(created.body).toLowerCase().includes('passwordhash'))
  check('register returns a public_id (ULID), not a numeric id',
  ...
  const dupe = await api('/api/auth/register', {
    method: 'POST',
    body: { firstName: 'T', lastName: 'B', email: customerEmail, password: 'Password123' },
  })
  check('duplicate email rejected with 409', dupe.status === 409, dupe.body?.error?.code)

  const wrongPass = await api('/api/auth/login', {
    method: 'POST', body: { email: customerEmail, password: 'WrongPassword1' },
  })
  const unknownEmail = await api('/api/auth/login', {
    method: 'POST', body: { email: `ghost.${rand}@x.com`, password: 'WrongPassword1' },
  })
  check('wrong password returns 401', wrongPass.status === 401)
  check('unknown email returns the SAME message (no enumeration)',
    wrongPass.body?.error?.message === unknownEmail.body?.error?.message,
    wrongPass.body?.error?.message)

  const login = await api('/api/auth/login', {
    method: 'POST', body: { email: customerEmail, password: 'Password123' },
  })
  check('login succeeds', login.status === 200)
  check('login sets an httpOnly refresh cookie',
...
section('AUTH — PROTECTED ROUTES')
{
  ...
  check('GET /me returns the profile', me.status === 200 && me.body.data.email === customerEmail)
  check('profile includes derived loyalty tier', typeof me.body?.data?.tier === 'string',
    me.body?.data?.tier)

  const patched = await api('/api/auth/me', {
    method: 'PATCH', token: customerToken,
    body: {
      firstName: 'Renamed', preferredSize: '9',
      address: { line1: '1 Test St', city: 'Mumbai', state: 'MH', postalCode: '400001', country: 'India' },
    },
  })
  check('PATCH /me updates the profile',
    patched.status === 200 && patched.body.data.firstName === 'Renamed')
  check('PATCH /me saves the address', patched.body?.data?.address?.city === 'Mumbai')

  // Mass-assignment: unknown keys must be stripped by zod, not persisted.
  const escalate = await api('/api/auth/me', {
    method: 'PATCH', token: customerToken, body: { role: 'admin', status: 'blocked' },
  })
  check('PATCH /me cannot escalate role', escalate.body?.data?.role === 'customer')

  const refreshed = await api('/api/auth/refresh', { method: 'POST', withCookies: true })
  check('POST /refresh issues a new access token',
    refreshed.status === 200 && Boolean(refreshed.body?.data?.accessToken))
  if (refreshed.body?.data?.accessToken) customerToken = refreshed.body.data.accessToken
}
...
{
  const login = await api('/api/auth/login', {
    method: 'POST', body: { email: 'admin@Kick.com', password: 'ChangeMe123!' },
  })
  ...
  check('customer CANNOT reach /api/admin/* (403)',
...
let sampleVariantId = null
{
  ...
  check('GET /api/products returns a list', list.status === 200 && Array.isArray(list.body.data))
  check('product list is paginated', typeof list.body?.meta?.totalPages === 'number',
  ...
  check('product id is a ULID, not a number',
    typeof first?.id === 'string' && first.id.length === 26)

  const filtered = await api('/api/products?gender=men&limit=5')
  check('filter by gender works',
    filtered.status === 200 && filtered.body.data.every((p) => p.gender === 'men'),
    `${filtered.body?.data?.length} men's products`)

  const priced = await api('/api/products?minPrice=150&maxPrice=200')
  check('filter by price range works',
    priced.status === 200 && priced.body.data.every((p) => p.price >= 150 && p.price <= 200))

  const sorted = await api('/api/products?sort=price_asc&limit=5')
  const prices = sorted.body?.data?.map((p) => p.price) ?? []
  check('sort=price_asc is ordered',
    prices.every((p, i) => i === 0 || p >= prices[i - 1]), prices.join(','))

  const search = await api('/api/products?q=runner')
  check('full-text search returns results', search.status === 200 && search.body.data.length > 0,
    `${search.body?.data?.length} hits`)

  const badSort = await api('/api/products?sort=DROP TABLE products')
  check('invalid sort is rejected by validation (no SQL injection)', badSort.status === 422)

  const featured = await api('/api/products/featured')
  check('GET /products/featured works', featured.status === 200 && featured.body.data.length > 0)

  const filters = await api('/api/products/filters')
  check('GET /products/filters returns sizes', Array.isArray(filters.body?.data?.sizes))
  ...
  check('variant exposes availability, not raw stock',
    inStockVariant && inStockVariant.available > 0 && inStockVariant.stock === undefined)
  const sizes = (detail.body?.data?.sizes ?? []).map(Number)
  check('sizes are sorted NUMERICALLY (not "10" before "9")',
  ...
  check('GET /api/categories works', categories.status === 200 && categories.body.data.length > 0,
    `${categories.body?.data?.length} categories`)
  check('category carries a product count',
...
section('CHECKOUT — QUOTE & VALIDATION')
{
  const quote = await api('/api/orders/quote', {
    method: 'POST', body: { items: [{ variantId: sampleVariantId, quantity: 2 }] },
  })
  check('POST /orders/quote prices the cart', quote.status === 200)
  check('quote computes subtotal, tax, shipping, total',
  ...
  check('tax is 8% of subtotal', Math.abs(quote.body.data.tax - expectedTax) < 0.02,
    `${quote.body.data.tax} vs ${expectedTax}`)

  const expectedTotal = Number(
    (quote.body.data.subtotal + quote.body.data.tax + quote.body.data.shipping).toFixed(2)
  )
  check('total = subtotal + tax + shipping',
    Math.abs(quote.body.data.total - expectedTotal) < 0.02)

  const empty = await api('/api/orders/quote', { method: 'POST', body: { items: [] } })
  ...
  const noEmail = await api('/api/orders', {
    ...
    body: {
      ...
      shippingAddress: { name: 'X', line1: '1 St', city: 'C', state: 'S', postalCode: '1', country: 'IN' },
      paymentMethod: 'cod',
    },
  })
...
let orderId = null
{
  ...
  const variantBefore = before.body.data.variants.find((v) => v.id === sampleVariantId)
  ...
  const order = await api('/api/orders', {
    ...
    body: {
      ...
      shippingAddress: {
        name: 'Test Buyer', phone: '+91 90000 00001', line1: '12 Marine Drive',
  ...
  check('POST /api/orders creates the order', order.status === 201, `status ${order.status}`)
  orderId = order.body?.data?.order?.id
  check('order id is a ULID', typeof orderId === 'string' && orderId.length === 26)
  check('order number looks like #1001', /^#\d+$/.test(order.body?.data?.order?.orderNumber ?? ''),
    order.body?.data?.order?.orderNumber)
  check('order starts pending / unpaid',
  ...
  const variantAfter = after.body.data.variants.find((v) => v.id === sampleVariantId)
  check('stock was RESERVED (available dropped by 2)',
  ...
  check('order contains its line items', detail.body?.data?.items?.length === 1)
  check('line item is a SNAPSHOT (has name + unit price)',
    Boolean(detail.body?.data?.items?.[0]?.name) &&
    typeof detail.body?.data?.items?.[0]?.unitPrice === 'number')
  check('customer never sees adminNote', detail.body?.data?.adminNote === undefined)
...
section('OWNERSHIP (IDOR)')
{
  ...
  const asAdmin = await api(`/api/admin/orders/${orderId}`, { token: adminToken })
...
section('INSUFFICIENT STOCK')
{
  const huge = await api('/api/orders', {
    ...
    body: {
      ...
      shippingAddress: {
        name: 'T', line1: '1 St', city: 'Mumbai', state: 'MH',
  ...
  check('large quantity is handled without a 500',
...
let adminProductId = null
{
  ...
  const created = await api('/api/admin/products', {
    ...
    body: {
      name: `Test Shoe ${rand}`, sku: `TEST-${rand.toUpperCase()}`,
      description: 'A product created by the automated end-to-end test suite for verification.',
      categoryId, gender: 'unisex', brand: 'Kick', material: 'Mesh',
      price: 99.99, compareAtPrice: 129.99, costPerItem: 40,
  ...
  const dupeSku = await api('/api/admin/products', {
    method: 'POST', token: adminToken,
    body: {
      name: 'Dupe', sku: `TEST-${rand.toUpperCase()}`,
      description: 'This SKU is a duplicate and must be rejected by the API.',
      categoryId, gender: 'unisex', brand: 'X', price: 10,
      colors: ['Black'], variants: [{ size: '8', stock: 1 }],
    },
  })
  ...
  const badCompare = await api('/api/admin/products', {
    method: 'POST', token: adminToken,
    body: {
      name: 'Bad', sku: `BAD-${rand.toUpperCase()}`,
      description: 'Compare-at price is lower than the selling price, which is invalid.',
      categoryId, gender: 'unisex', brand: 'X', price: 100, compareAtPrice: 50,
      colors: ['Black'], variants: [{ size: '8', stock: 1 }],
    },
  })
  ...
  const updated = await api(`/api/admin/products/${adminProductId}`, {
    method: 'PATCH', token: adminToken, body: { price: 89.99, featured: true },
  })
  check('PATCH /admin/products/:id updates', updated.status === 200 && updated.body.data.price === 89.99)
  ...
  const detail = await api(`/api/admin/products/${adminProductId}`, { token: adminToken })
  check('admin detail includes sales stats', typeof detail.body?.data?.sales?.unitsSold === 'number')
...
let adminCategoryId = null
{
  const created = await api('/api/admin/categories', {
    method: 'POST', token: adminToken,
    body: { name: `Test Cat ${rand}`, description: 'Created by the test suite.', color: 'violet' },
  })
  check('POST /admin/categories creates', created.status === 201)
  adminCategoryId = created.body?.data?.id
  check('slug is generated from the name',
    created.body?.data?.slug?.startsWith('test-cat'), created.body?.data?.slug)

  const assigned = await api(`/api/admin/categories/${adminCategoryId}/products`, {
    method: 'POST', token: adminToken, body: { productIds: [adminProductId] },
  })
  check('bulk-assign products to a category', assigned.status === 200 && assigned.body.data.assigned === 1)

  const products = await api(`/api/admin/categories/${adminCategoryId}/products`, { token: adminToken })
  check('category product list reflects the assignment', products.body?.data?.length === 1)

  const updated = await api(`/api/admin/categories/${adminCategoryId}`, {
    method: 'PATCH', token: adminToken, body: { color: 'rose' },
  })
  check('PATCH /admin/categories/:id works', updated.body?.data?.color === 'rose')

  const badColor = await api('/api/admin/categories', {
    method: 'POST', token: adminToken, body: { name: 'Bad Colour', color: 'neon' },
  })
...
section('ADMIN — ORDERS')
{
  ...
  check('GET /api/admin/orders works', list.status === 200 && list.body.data.length > 0)

  const filtered = await api('/api/admin/orders?status=pending', { token: adminToken })
  check('filter orders by status',
    filtered.status === 200 && filtered.body.data.every((o) => o.status === 'pending'))
  ...
  check('illegal status jump (pending -> delivered) rejected',
    illegal.status === 400, illegal.body?.error?.message?.slice(0, 60))
  ...
  check('shipping without a tracking number is rejected', noTracking.status === 422)

  const shipped = await api(`/api/admin/orders/${orderId}/status`, {
    method: 'PATCH', token: adminToken,
    body: { status: 'shipped', courier: 'BlueDart', trackingNumber: 'BD1234567890' },
  })
  ...
  const note = await api(`/api/admin/orders/${orderId}/note`, {
    method: 'PATCH', token: adminToken, body: { adminNote: 'Verified by test suite.' },
  })
  check('admin note can be set', note.status === 200)

  const csv = await api('/api/admin/orders/export', { token: adminToken })
  check('order CSV export works', csv.status === 200 && String(csv.body?.raw ?? '').startsWith('Order,Date'))
}

section('ADMIN — CUSTOMERS')
{
  const list = await api('/api/admin/customers', { token: adminToken })
  check('GET /api/admin/customers works', list.status === 200 && list.body.data.length > 0,
    `${list.body?.meta?.total} customers`)

  const first = list.body.data.find((c) => c.email === customerEmail)
  check('customer row carries derived tier', typeof first?.tier === 'string', first?.tier)
  check('customer row carries order stats', typeof first?.totalOrders === 'number',
    `${first?.totalOrders} orders, spent ${first?.totalSpent}`)
  check('admin customer list exposes the ULID, never the numeric id',
    typeof first?.id === 'string' && first.id.length === 26 && first.internalId === undefined,
    first?.id)

  const searched = await api(`/api/admin/customers?q=${encodeURIComponent('Renamed')}`, { token: adminToken })
  check('customer search works', searched.status === 200 && searched.body.data.length >= 1)

  const detail = await api(`/api/admin/customers/${first.publicId}`, { token: adminToken })
  check('GET /admin/customers/:id works', detail.status === 200)
  check('detail includes favourite products', Array.isArray(detail.body?.data?.favouriteProducts))

  const orders = await api(`/api/admin/customers/${first.publicId}/orders`, { token: adminToken })
  check('customer order history works', orders.status === 200 && orders.body.data.length >= 1)

  const blocked = await api(`/api/admin/customers/${first.publicId}/status`, {
    method: 'PATCH', token: adminToken, body: { status: 'blocked' },
  })
  check('admin can block a customer', blocked.status === 200 && blocked.body.data.status === 'blocked')

  const afterBlock = await api('/api/auth/me', { token: customerToken })
  check('a blocked customer is rejected immediately (403)', afterBlock.status === 403)
  ...
  check('customer CSV export works', csv.status === 200 && String(csv.body?.raw ?? '').startsWith('ID,First Name'))
}

section('ADMIN — DASHBOARD')
{
  const overview = await api('/api/admin/dashboard/overview', { token: adminToken })
  check('GET /admin/dashboard/overview works', overview.status === 200)
  check('overview has stats + charts',
    Boolean(overview.body?.data?.stats) && Array.isArray(overview.body?.data?.revenueChart))

  const stats = await api('/api/admin/dashboard/stats', { token: adminToken })
  const s = stats.body?.data
  check('stats include revenue, orders, customers, AOV',
    typeof s?.totalRevenue === 'number' && typeof s?.totalOrders === 'number' &&
    typeof s?.totalCustomers === 'number' && typeof s?.avgOrderValue === 'number',
    `revenue ${s?.totalRevenue}, orders ${s?.totalOrders}`)
  check('stats include fulfilment counters',
    typeof s?.pendingOrders === 'number' && typeof s?.deliveredOrders === 'number')
  check('stats include period-over-period change',
    typeof s?.revenueChange === 'number')

  for (const path of [
    'revenue', 'daily', 'revenue-by-category', 'sales-by-size',
    'orders-by-status', 'recent-orders', 'top-products', 'low-stock',
  ]) {
    const r = await api(`/api/admin/dashboard/${path}`, { token: adminToken })
    check(`GET /admin/dashboard/${path}`, r.status === 200 && Array.isArray(r.body.data))
  }
}
...
section('CLEANUP + DELETE BEHAVIOUR')
{
  ...
  const gone = await api(`/api/admin/products/${adminProductId}`, { token: adminToken })
  ...
  const logout = await api('/api/auth/logout', { method: 'POST', withCookies: true })
...
section('CONCURRENCY — THE OVERSELL TEST')
{
  ...
  const scarce = await api('/api/admin/products', {
    method: 'POST', token: adminToken,
    body: {
      name: `Last Pair ${rand}`, sku: `LAST-${rand.toUpperCase()}`,
      description: 'A product with exactly one pair in stock, for the concurrency test.',
      categoryId, gender: 'unisex', brand: 'Kick',
      price: 50, status: 'active', featured: false,
      colors: ['Black'], variants: [{ size: '9', stock: 1 }],
    },
  })
  check('created a product with stock = 1', scarce.status === 201 &&
    scarce.body?.data?.totalStock === 1, `stock ${scarce.body?.data?.totalStock}`)

  const detail = await api(`/api/products/${scarce.body.data.slug}`)
  const variantId = detail.body?.data?.variants?.[0]?.id

  const buy = () => api('/api/orders', {
    method: 'POST', token: customerToken,
    body: {
      items: [{ variantId, quantity: 1 }],
      shippingAddress: {
        name: 'Racer', line1: '1 St', city: 'Mumbai', state: 'MH',
        postalCode: '400001', country: 'India',
      },
      paymentMethod: 'cod',
    },
  ...
  check('exactly ONE checkout succeeded, one got 409 INSUFFICIENT_STOCK',
    statuses[0] === 201 && statuses[1] === 409,
    `statuses ${statuses.join(' & ')}`)

  const after = await api(`/api/products/${scarce.body.data.slug}`)
  check('no overselling — available is now 0',
