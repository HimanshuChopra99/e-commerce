...
const customer = await api('/api/auth/register', {
  ...
  body: {
    ...
    email: `load.${rand}@example.com`, password: 'Password123',
  },
})
...
console.log('\n=== READ THROUGHPUT (200 concurrent product reads) ===')
{
  ...
  check('sustained throughput above 150 req/s', N / (wall / 1000) > 150,
...
console.log('\n=== PER-REQUEST LATENCY (sequential, no queueing) ===')
{
  ...
  check('p50 latency under 50ms', percentile(times, 50) < 50,
    `${percentile(times, 50).toFixed(1)}ms`)
  check('p95 latency under 150ms', percentile(times, 95) < 150,
...
console.log('\n=== OVERSELL UNDER LOAD (20 buyers, 5 pairs) ===')
{
  ...
  const product = await api('/api/admin/products', {
    ...
    body: {
      name: `Race Stock ${rand}`, sku: `RACE-${rand.toUpperCase()}`,
      description: 'Five pairs in stock, twenty simultaneous buyers. Exactly five must win.',
      categoryId, gender: 'unisex', brand: 'Kick',
      price: 75, status: 'active', featured: false,
      colors: ['Black'], variants: [{ size: '9', stock: 5 }],
    },
  })
  check('created product with stock = 5', product.body?.data?.totalStock === 5,
  ...
  console.log(`        ${created} created · ${rejected} rejected · ${errors} server errors · ${wall.toFixed(0)}ms`)

  check('exactly 5 orders succeeded', created === 5, `${created} created`)
  check('the other 15 were cleanly rejected with 409', rejected === 15, `${rejected} rejected`)
  check('no 500s under contention', errors === 0)

  const after = await api(`/api/products/${product.body.data.slug}`)
  check('stock is exactly 0 — nothing oversold',
...
console.log('\n=== CONNECTION POOL (300 concurrent DB-backed requests) ===')
{
  ...
  const t0 = performance.now()
  const results = await Promise.all(
    Array.from({ length: N }, () => api('/api/admin/dashboard/stats', { token: adminToken }))
  )
  const wall = performance.now() - t0
  const oks = results.filter((r) => r.status === 200).length

  check('pool handled 300 concurrent queries without exhaustion', oks === N, `${oks}/${N}`)
  console.log(`        wall ${wall.toFixed(0)}ms · ${(N / (wall / 1000)).toFixed(0)} req/s`)
}

console.log(`\n${'='.repeat(60)}`)
console.log(failures === 0 ? '  LOAD TEST PASSED' : `  ${failures} CHECK(S) FAILED`)
