...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    setError(null)
    try {
      const data = await api.post('/delivery-partner/login', { email, password })
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-margin-mobile">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <p className="text-body-md text-on-surface-variant mt-1">Delivery Partner Portal</p>
        </div>

        <form onSubmit={handleLogin} className="bg-surface-container-lowest rounded-2xl p-6 border border-surface-container-highest shadow-sm flex flex-col gap-4">
