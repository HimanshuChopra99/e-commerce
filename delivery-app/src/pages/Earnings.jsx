    ...
    <section className="flex flex-col gap-md">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-[#7b46e0] to-[#9a6bff] text-white shadow-lg shadow-primary/25 p-md">
        ...
        <div className="relative z-10 flex flex-col gap-sm">
          ...
          <div className="flex items-baseline gap-xs">
            <span className="text-[34px] leading-none font-bold tracking-tight">{formatMoney(stats.earnings)}</span>
          </div>

          <div className="mt-1">
            <div className="flex justify-between text-label-sm mb-1 opacity-90">
              <span>Today</span>
              <span className="font-semibold">{formatMoney(stats.earningsToday)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <div className="bg-surface-container-lowest rounded-2xl p-3 border border-surface-container-highest shadow-sm flex flex-col">
          ...
          <span className="mt-1.5 text-[11px] text-on-surface-variant">Completed</span>
        </div>
        ...
        <div className="bg-surface-container-lowest rounded-2xl p-3 border border-surface-container-highest shadow-sm flex flex-col">
          ...
          <span className="mt-1.5 text-[11px] text-on-surface-variant">Active</span>
        </div>
      </div>
    </section>
  )
              ...
              >
                <div className="flex items-center gap-md">
                  ...
                  <div className="flex flex-col">
                    <span className="text-body-lg font-bold text-on-surface">Order {d.orderNumber}</span>
                    <span className="text-label-sm text-on-surface-variant capitalize">{d.status}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  <span className="text-body-lg font-bold text-on-surface">{formatMoney(d.payout)}</span>
                </div>
              </li>
            ))}
          </ul>
