...
export default function OrderCard({ order, onAccept, onReject }) {
  ...
  const itemCount    = order?.items ?? order?.itemCount ?? order?.item_count ?? '?'
  const total        = order?.total != null ? `$${Number(order.total).toFixed(2)}` : '—'
  const payout       = order?.payout 
    ? (typeof order.payout === 'number' ? `$${Number(order.payout).toFixed(2)}` : order.payout) 
    : total

  const customerName = order?.customer ?? order?.customerName ?? order?.customer_name ?? null
  ...
  if (!distance || distance === '—') {
    ...
    if (Number.isFinite(sLat) && Number.isFinite(sLng)) {
      ...
      const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(pLat * (Math.PI / 180)) * Math.cos(sLat * (Math.PI / 180)) * Math.sin(dLon / 2) * Math.sin(dLon / 2)
      const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a))
      const roadKm = Math.max(0.6, R * c * 1.3)
      distance = roadKm < 1 ? `${Math.round(roadKm * 1000)} m` : `${roadKm.toFixed(1)} km`
    ...
    <article className="bg-surface-container-lowest rounded-[20px] p-md shadow-[0px_4px_20px_0px_rgba(0,0,0,0.04)] border border-surface-container-low flex flex-col gap-sm">
      ...
      <div className="flex justify-between items-center border-b border-surface-variant pb-xs">
        <div className="flex items-center gap-xs">
          ...
          <h2 className="text-body-lg font-bold text-on-surface">#{orderNumber}</h2>
        </div>
      ...
      <div className="flex items-center justify-between gap-sm pt-xs pb-xs">
        <div className="flex gap-md items-start min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-body-md font-bold text-on-surface truncate">{destination}</h3>
            {customerName && <p className="text-label-sm text-on-surface-variant truncate">Customer: {customerName}</p>}
          </div>
        </div>
