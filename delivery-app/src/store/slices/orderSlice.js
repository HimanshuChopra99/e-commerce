...
const orderSlice = createSlice({
  ...
  reducers: {
    ...
    orderBroadcasted: (state, action) => {
      ...
      const exists = state.availableOrders.find(o => o.id === action.payload.orderId)
      if (!exists) {
    ...
    orderTakenAway: (state, action) => {
      ...
      state.availableOrders = state.availableOrders.filter(o => o.id !== action.payload.orderId)
    },
    acceptOrderSuccess: (state, action) => {
      ...
      const foundInAvailable = state.availableOrders.find(
        o => o.id === (action.payload?.id || action.payload?.publicId) || o.orderNumber === action.payload?.orderNumber
      )
      const enrichedOrder = {
        ...foundInAvailable,
        ...action.payload,
        id: action.payload?.id || action.payload?.publicId || foundInAvailable?.id,
        orderNumber: action.payload?.orderNumber || foundInAvailable?.orderNumber || action.payload?.id,
        customerName: action.payload?.customerName || foundInAvailable?.customerName || 'Customer',
        pickupAddress: action.payload?.pickupAddress || foundInAvailable?.pickupAddress || 'KICKS Main Hub',
        pickupLat: action.payload?.pickupLat ?? foundInAvailable?.pickupLat ?? state.warehouseLocation?.lat,
        pickupLng: action.payload?.pickupLng ?? foundInAvailable?.pickupLng ?? state.warehouseLocation?.lng,
        dropoffAddress: foundInAvailable?.dropoffAddress || [
          action.payload?.shippingAddress?.city,
          action.payload?.shippingAddress?.state,
        ].filter(Boolean).join(', ') || 'Customer Location',
        shippingAddress: action.payload?.shippingAddress || foundInAvailable?.shippingAddress,
        distance: foundInAvailable?.distance || action.payload?.distance || '2.8 km',
        eta: foundInAvailable?.eta || action.payload?.eta || '12 min',
        payout: foundInAvailable?.payout || action.payload?.payout || Number((Number(action.payload?.total || 0) * 0.08).toFixed(2)) || 10,
        total: action.payload?.total ?? foundInAvailable?.total,
        itemCount: action.payload?.itemCount ?? foundInAvailable?.itemCount ?? (action.payload?.items?.length || 1),
      }
