...
function notifyCartAction(lastAction, items) {
  try {
    ...
    const payload = {
      lastAction,
      cartSummary: { totalItems, totalPrice, items: summaryItems },
    };
...
function addLocal(items, payload) {
  const { variantId, productId, name, image, price, size, color, slug } =
    payload;
  ...
  async (_, { getState, rejectWithValue }) => {
    ...
    try {
      ...
      const response = guestItems.length
        ? await cartApi.sync(
            guestItems.map(({ variantId, quantity }) => ({
              variantId,
              quantity,
            }))
          )
...
export const removeFromCart = createAsyncThunk(
  ...
  async (variantId, { getState, rejectWithValue }) => {
    const { user } = getState().auth;
    if (!user) {
      const items = getState().cart.items.filter(
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(addToCart.fulfilled, (state, action) => {
        ...
        notifyCartAction(
          `Added ${action.meta?.arg?.name || 'item'} to cart`,
      ...
      .addCase(updateQuantity.fulfilled, (state, action) => {
        ...
        notifyCartAction(
          `Updated cart item quantity to ${qty}`,
          action.payload.items
        );
      })
