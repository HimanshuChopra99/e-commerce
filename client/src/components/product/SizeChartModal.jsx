...
export const SizeChartModal = ({
  ...
  return <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
  ...
  >
    ...
    /* Header */
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-500">Adidas 4DFWD X Parley Standard Unisex Sizing</p>
          </div>
        </div>
    ...
    /* Toggle Unit */
        ...
        <div className="flex justify-between items-center mb-4 bg-neutral-100 p-1.5 rounded-xl text-xs font-bold">
          <span className="text-neutral-600 px-2">Foot Length Measurement:</span>
  ...
  >
    ...
    const footLength = unit === "cm" ? `${s.cm} cm` : `${(s.cm / 2.54).toFixed(1)} in`;
    return <tr
      key={s.value}
      className={`hover:bg-neutral-50 transition-colors ${isCurrent ? "bg-blue-50/60 font-bold" : ""}`}
    >
                    <td className="py-3 px-4 text-neutral-900 font-bold">EU {s.eu}</td>
    ...
    >
                        {isCurrent ? <Check className="w-3.5 h-3.5 inline" /> : "Select"}
                      </button>
                    </td>
                  </tr>;
  })}
    ...
    /* Measurement Advice Box */
        ...
        <div className="bg-amber-50/70 border border-amber-200 rounded-2xl p-4 text-xs text-amber-900">
          ...
          <p className="leading-relaxed">
            The adidas 4DFWD Primeknit upper offers a snug, sock-like compression fit. If you prefer extra wiggle room in the toe box or wear thicker running socks, we recommend sizing up by half a size.
          </p>
        </div>
