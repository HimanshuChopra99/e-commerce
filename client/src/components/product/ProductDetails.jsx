    ...
    <div className="w-full flex flex-col space-y-6">
          ...
          {product.colors.map((color) => {
            ...
            return (
              ...
              >
                {isSelected && (
                  <Check
                    className={`w-4 h-4 ${color.id === 'shadow-navy' ? 'text-white' : 'text-neutral-900'}`}
                  />
                )}
              </button>
            );
      ...
      <div className="pt-4 border-t border-neutral-200/80 space-y-3.5 text-neutral-800">
        ...
        <p className="text-xs text-neutral-700 leading-relaxed font-normal bg-white/70 p-3.5 rounded-xl border border-neutral-200/70 shadow-2xs">
          {product.description ||
            'High-performance sneakers engineered for comfort, durability, and athletic mobility.'}
        </p>

        {/* Selected Color & Key Highlights */}
        <div className="flex items-center gap-2 text-xs font-semibold text-neutral-800">
          <span className="text-neutral-500 uppercase text-[11px] tracking-wider">
            Color:
          </span>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs pt-1">
          {product.specs?.slice(0, 4).map((spec, i) => (
            <div
              key={i}
              className="flex justify-between items-center bg-white/50 border border-neutral-200/60 p-2 rounded-lg text-[11px]"
            >
        ...
        <ul className="space-y-2 text-xs text-neutral-700 font-medium pt-1">
          ...
          <li className="flex items-start gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-neutral-900 mt-1.5 shrink-0" />
            <span>
              Unlimited free standard shipping, 30-day hassle-free returns &
              exchanges.
            </span>
          </li>
        </ul>
