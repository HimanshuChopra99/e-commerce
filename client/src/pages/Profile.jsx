...
export default function Profile() {
    ...
    return (
      <section className="mx-auto max-w-md px-4 py-20 text-center">
        ...
        <p className="mt-2 text-sm text-slate-500 leading-relaxed">
          Please sign in to access your saved cart, favourites, profile details,
          and order history.
        </p>
        <Link
  ...
  const confirmLogout = async () => {
    ...
    setLoggingOut(false);
    setLogoutOpen(false);
    showToast('You have been logged out safely.', 'profile', {
      title: 'Signed out',
    });
          ...
          <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-6 border-t border-slate-100 pt-4 sm:border-t-0 sm:pt-0">
              ...
              <Link to="/cart" className="group text-left sm:text-right">
                <p className="text-xs font-medium text-slate-500 group-hover:text-blue-600 transition">
                  Cart Items
                </p>
                <p className="text-lg font-bold text-slate-900 group-hover:text-blue-600 transition">
                  {cart.length}
                </p>
              </Link>
            </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Pre-filled automatically during checkout.
              </p>
            </div>
          ...
          {hasPin && (
            <div className="mt-3 inline-flex items-center gap-2 rounded-md bg-emerald-50 border border-emerald-200 px-3 py-1.5 text-xs text-emerald-700 font-medium">
              ...
              <span>
                Exact Doorstep Pin Selected (
                {Number(form.address.lat).toFixed(4)},{' '}
                {Number(form.address.lng).toFixed(4)})
              </span>
            </div>
          )}
    ...
    >
      ...
      <div className="relative w-full max-w-sm rounded-xl bg-white p-6 shadow-xl border border-slate-200">
        ...
        <p className="mt-2 text-xs text-slate-500 leading-relaxed">
          Are you sure you want to log out? Your items and cart will remain
