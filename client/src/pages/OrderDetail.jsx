...
export default function OrderDetail() {
  ...
  const reduxOrder = orders.find(
    (o) =>
      ...
      String(o.orderNumber || '').replace('#', '') === String(id).replace('#', '')
  )
  ...
  const isLoading = (loading && orders.length === 0) || (directLoading && !order)
    ...
    return (
      <div className='min-h-[75vh] bg-white py-20 flex items-center justify-center'>
        <div className='mx-auto max-w-md px-4 text-center'>
          ...
          <h2 className='mt-4 text-xl font-bold text-slate-900'>Order Not Found</h2>
        ...
        <div className='flex flex-col justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end'>
          <div>
            <div className='flex items-center gap-2 text-xs font-medium text-slate-500 mb-2'>
              ...
              <span>/</span>
              <span className='text-slate-900 font-semibold'>#{order.orderNumber || order.id}</span>
            </div>
            ...
            <p className='text-xs text-slate-500 mt-1.5 flex items-center gap-1.5'>
              ...
              {new Date(order.placedAt || order.createdAt).toLocaleDateString(undefined, { dateStyle: 'long' })}
            </p>
          </div>
              ...
              <div className='grid gap-8 sm:grid-cols-2'>
                <div className='space-y-2'>
                  <div className='flex items-center gap-2 text-xs font-semibold text-slate-700'>
                    <MapPin className='size-4 text-slate-400' /> Destination Address
                  </div>
                  {order.shippingAddress ? (
                    ...
                    <p className='text-xs text-slate-500 pl-6'>Standard profile address assigned.</p>
                  )}
                </div>

                <div className='space-y-2'>
                  <div className='flex items-center gap-2 text-xs font-semibold text-slate-700'>
                    <Truck className='size-4 text-slate-400' /> Logistics & Shipping Method
                  </div>
                  <div className='text-xs text-slate-600 leading-relaxed space-y-1.5 pl-6 border-l-2 border-slate-200'>
                    <p className='flex justify-between'>
                      ...
                      <span className='font-semibold text-slate-900'>{order.courier || 'Express Ground'}</span>
                    </p>
                    <p className='flex justify-between'>
            ...
            <div className='relative mx-auto w-full max-w-sm rounded-t-xl bg-white p-6 pb-8 shadow-xl border border-neutral-200/80 text-neutral-800 font-mono text-xs overflow-hidden sm:overflow-visible'>
              ...
              <div className='space-y-1.5 border-b-2 border-dashed border-neutral-300 pb-4 mb-4 text-[11px] text-neutral-600'>
                <div className='flex justify-between'>
                  <span className='text-neutral-400 uppercase'>Receipt No:</span>
                  <span className='font-bold text-neutral-900'>#{order.orderNumber || order.id}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-neutral-400 uppercase'>Date & Time:</span>
                ...
                <div className='flex justify-between'>
                  ...
                  <span className='font-medium text-neutral-800'>{order.paymentMethod || 'STRIPE / CARD'}</span>
                </div>
                <div className='flex justify-between'>
              ...
              <div className='space-y-1.5 text-[11px] text-neutral-600 border-b-2 border-dashed border-neutral-300 pb-4 mb-4'>
                <div className='flex justify-between'>
                  ...
                  <span className='font-medium text-neutral-900'>
                    {money(order.subtotal || (order.total || order.grandTotal) * 0.9, order.currency)}
                  </span>
                </div>

                <div className='flex justify-between'>
                  <span className='text-neutral-400 uppercase'>SHIPPING & HANDLING</span>
                  <span className='font-medium text-neutral-900'>
                ...
                <div className='flex justify-between'>
                  <span className='text-neutral-400 uppercase'>ESTIMATED TAX</span>
                  <span className='font-medium text-neutral-900'>
                ...
                <div className='pt-2 flex justify-between items-baseline font-black text-sm text-neutral-900 border-t border-neutral-200 mt-2'>
                  <span className='uppercase tracking-wider'>TOTAL CHARGED</span>
              ...
              <div className='text-center pt-1 space-y-3'>
                ...
                <p className='text-[10px] text-neutral-500 leading-normal uppercase font-sans font-medium pt-1'>
                  THANK YOU FOR SHOPPING WITH KICKS.<br />ALL RETURNS SUBJECT TO STORE POLICY.
                </p>
              </div>
        ...
        return (
          <React.Fragment key={step.label}>
            <div className='flex flex-col sm:flex-row items-center gap-2 text-center sm:text-left'>
              <span
                className={`flex size-8 items-center justify-center rounded-full text-xs font-bold transition ${isCompleted
              ...
              <span
                className={`text-xs font-semibold ${isCompleted ? 'text-slate-900' : 'text-slate-400'
                  }`}
              >
                {step.label}
              </span>
            ...
            {index < steps.length - 1 && (
              <div
                className={`h-0.5 flex-1 mx-2 sm:mx-4 transition ${stepNum < currentStep ? 'bg-slate-900' : 'bg-slate-200'
                  }`}
              />
            )}
          </React.Fragment>
