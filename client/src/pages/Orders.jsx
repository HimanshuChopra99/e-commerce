      ...
      <div className='min-h-[75vh] bg-slate-50/60 py-16 flex items-center justify-center'>
        <div className='mx-auto max-w-md px-4 text-center'>
          ...
          <p className='mt-2 text-sm text-slate-500 leading-relaxed'>
            Please sign in to view your order history, track live shipments, and access receipts.
          </p>
          <button
        ...
        <div className='flex flex-col justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-center'>
          <div>
            <div className='flex items-center gap-2 text-xs font-medium text-slate-500 mb-1'>
              <Link to='/profile' className='hover:text-slate-900 transition flex items-center gap-1'>
            ...
            <div className='flex items-start gap-4'>
              ...
              <div className='min-w-0 flex-1'>
                ...
                <p className='mt-1 text-xs text-emerald-800 leading-relaxed'>
                  Order total of <strong>{money(justPlaced.total || justPlaced.grandTotal)}</strong> confirmed.
                  {justPlaced.customerEmail && ` Details sent to ${justPlaced.customerEmail}.`}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className='mt-8'>
          {loading ? (
            <div className='space-y-4'>
              {[1, 2, 3].map((key) => (
                <div key={key} className='h-32 animate-pulse rounded-xl bg-slate-200/60' />
              ))}
            </div>
          ) : error ? (
            ...
            <div className='rounded-xl border border-slate-200 bg-white py-16 text-center shadow-sm'>
              ...
              <h3 className='mt-4 text-sm font-semibold text-slate-900'>No Orders Placed Yet</h3>
              <p className='mt-1 text-xs text-slate-500 max-w-sm mx-auto'>
                Your purchase history is empty. Products you order will appear here.
              </p>
              <button
                onClick={() => navigate('/products')}
