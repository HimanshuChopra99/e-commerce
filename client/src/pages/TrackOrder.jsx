...
async function updateRoadRoute(map, startLatLng, endLatLng, polylineRef) {
  ...
  try {
    ...
    if (data.code === 'Ok' && data.routes?.[0]?.geometry?.coordinates) {
      const roadPath = data.routes[0].geometry.coordinates.map(([lng, lat]) => [
        lat,
        lng,
      ]);
    ...
    console.warn(
      'Road routing fetch failed, falling back to direct line:',
      err
    );
    drawFallbackLine();
  }
...
export default function TrackOrder() {
  ...
  useEffect(() => {
    ...
    if (curr?.lat && curr?.lng) {
      partnerMarkerRef.current = L.marker([curr.lat, curr.lng], {
        icon: partnerIcon,
      })
        .addTo(map)
    ...
    if (dest?.lat && dest?.lng && curr?.lat && curr?.lng) {
      ...
      map.fitBounds(
        L.latLngBounds([dest.lat, dest.lng], [curr.lat, curr.lng]).pad(0.3)
      );
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-[#1E1E1E]">
                Live Tracking
              </h1>
              {partnerOnline && (
          ...
          <div className="absolute inset-0 flex items-end justify-center pb-16 pointer-events-none z-[999]">
            <div className="bg-white/95 backdrop-blur rounded-xl shadow-lg px-4 py-3 text-center max-w-xs mx-4">
              ...
              <p className="text-xs font-semibold text-gray-700">
                Partner location pending
              </p>
              <p className="text-xs text-gray-400 mt-0.5">
                Map will update when partner starts delivery
              </p>
            </div>
          </div>
        )}
